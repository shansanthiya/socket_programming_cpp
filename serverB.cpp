...
int main()
{
	...
	cout << "\nThe ServerB is up and running using UDP on port <" << portNumA << ">" << endl;
	...
	{	
		...
		cout << "\nThe ServerB recieved input <" << word << "> and operation <" << func << ">" << endl;
		...
		if(func.compare("prefix") == 0)
		{
			...
			cout << "\nThe ServerB has found <" << buf_ser_aws << "> matches" << endl;
			
			strcpy( buf_ser_aws, prefix_str.c_str());
			sendto(serverA_fd, buf_ser_aws, bufsize, 0,(struct sockaddr*)&serverA_aws,sizeof(serverA_aws));
			//cout << "\nThe ServerB sent <" << buf_ser_aws << "> to AWS" << endl;
		...
		else if(func.compare("search") == 0)
		{
			...
			sendto(serverA_fd, buf_ser_aws, bufsize, 0,(struct sockaddr*)&serverA_aws,sizeof(serverA_aws));
			if (match > 0 )
			cout << "\nThe ServerB has found <1> match and <" << buf_ser_aws << "> similar words" << endl;
			else
			cout << "\nThe ServerB has found <0> match and <" << buf_ser_aws << "> similar words" << endl;
		...
		cout << "\nThe ServerB finished sending the output to AWS" << endl;
