...
int main()
{
	...
	cout << "\nThe ServerC is up and running using UDP on port <" << portNumA << ">" << endl;
	...
	while(1)
	{
		...
		cout << "\nThe ServerC recieved input <" << word << "> and operation <" << func << ">" << endl;
		...
		if(func.compare("prefix") == 0)
		{
			...
			cout << "\nThe ServerC has found <" << buf_ser_aws << "> matches" << endl;
			
			strcpy( buf_ser_aws, prefix_str.c_str());
			sendto(serverA_fd, buf_ser_aws, bufsize, 0,(struct sockaddr*)&serverA_aws,sizeof(serverA_aws));
			//cout << "\nThe ServerC sent <" << buf_ser_aws << "> to AWS" << endl;
		...
		else if(func.compare("search") == 0)
		{
			...
			sendto(serverA_fd, buf_ser_aws, bufsize, 0,(struct sockaddr*)&serverA_aws,sizeof(serverA_aws));
			if (match > 0 )
			cout << "\nThe ServerC has found <1> match and <" << buf_ser_aws << "> similar words" << endl;
			else
			cout << "\nThe ServerC has found <0> match and <" << buf_ser_aws << "> similar words" << endl;
		...
		cout << "\nThe ServerC finished sending the output to AWS" << endl;
...
int suffix(string s1)
{
	...
	//cout << "Enter suffix to be searched: " ;
	//cin >> s1;
	int l1 = s1.length();
	transform(s1.begin(), s1.end(), s1.begin(), ::tolower);
